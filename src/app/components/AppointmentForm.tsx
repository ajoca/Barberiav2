...
export default function AppointmentForm({
  ...
  // Cargar servicios
  useEffect(() => {
    (async () => setServices(await db.services.toArray()))();
  }, []);

  // Reset de byweekday si corresponde
  useEffect(() => {
      ...
      >
        ...
        >
          ...
          <div className="p-4 space-y-4">
            {error && (
              <motion.p
                ...
                className="rounded-md border border-red-400/60 bg-red-50 text-red-700
            ...
            <AnimatePresence>
              {isRecurring && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -6 }}
                  className="space-y-3 rounded-xl border border-border/60 bg-secondary p-3"
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label className="block">
                      ...
                      {days.map(({ label, value }) => {
                        ...
                        return (
                          <motion.button
                            ...
                            className={`chip ${isActive ? 'chip-on' : 'chip-off'}`}
                          >
                            {label}
                          </motion.button>
                        );
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end p-4 border-t border-border/60">
            ...
            <motion.button whileTap={{ scale: 0.985 }} className="btn-primary">
              Guardar
            </motion.button>
          </div>
        </motion.form>
          ...
          {showDeleteModal && (
            ...
            >
              <motion.div
                ...
                className="w-[min(92vw,28rem)] card p-6"
              >
                ...
                <div className="mt-4 space-y-2">
                  <button onClick={() => confirmDelete('uno')} className="btn-primary w-full">
                    Cancelar solo este
                  </button>
                  <button onClick={() => confirmDelete('todos')} className="btn-danger w-full">
